...
<div class="table-responsive">
  <table class="table table-striped table-hover">
    ...
    <?php if(is_array($info) || $info instanceof \think\Collection || $info instanceof \think\Paginator): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
      <tr>
        <td class="td_id">
        	<input class="id_input" type="hidden" value="<?php echo $val['id']; ?>" />
        	<?php echo $val['id']; ?>
        </td>
...
<script type="text/javascript">
	var checkall = document.getElementsByClassName("del"); 
	var check_al = [];
	for(var i = 0;i < checkall.length ;i++){
		check_al.push($(checkall[i]).attr("name"));
	}
	console.log(check_al);
	$('.del').click(function(){
